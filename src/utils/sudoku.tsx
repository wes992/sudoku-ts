...
interface LocationSafetyItems {
  puzzleArray: number[][];
  emptyCell: EmptyCell;
  num: number;
}

const rowSafe = (props: LocationSafetyItems) => {
  const { puzzleArray, emptyCell, num } = props;
  // const es6Test = !puzzleArray[emptyCell.rowIndex].includes(num);
  // -1 is return value of .find() if value not found
  // return puzzleArray[emptyCell.rowIndex].indexOf(num) == -1;
  return !puzzleArray[emptyCell.rowIndex].includes(num);
};
const colSafe = (props: LocationSafetyItems) => {
  const { puzzleArray, emptyCell, num } = props;
  return !puzzleArray.some((row) => row[emptyCell.colIndex] == num);
};

const boxSafe = (props: LocationSafetyItems) => {
  const { puzzleArray, emptyCell, num } = props;
...
const safeToPlace = (props: LocationSafetyItems) => {
  return rowSafe(props) && colSafe(props) && boxSafe(props);
};

/*----------------- Obtain Next Empty Cell ------------------*/

const nextEmptyCell = (puzzleArray: Board) => {
  const emptyCell: EmptyCell = { rowIndex: -1, colIndex: -1 };
...
const fillPuzzle = (startingBoard: Board) => {
  ...
  for (const num of shuffle(numArray)) {
    ...
    if (safeToPlace({ puzzleArray: startingBoard, emptyCell, num })) {
...
interface RemovedVal extends EmptyCell {
  val: number;
}

type Result = {
  removedVals: RemovedVal[];
  startingBoard: Board;
};

const pokeHoles = (startingBoard: Board, holes: number) => {
  const removedVals: RemovedVal[] = [];
  ...
  const result: Result = { removedVals, startingBoard };
  return result;
};

/*---------------- Initialize -------------------- */

interface NewStartingBoard extends Result {
  solvedBoard: Board;
}

type levels = "easy" | "medium" | "hard" | "very-hard" | "insane" | "inhuman";

export const newStartingBoard = (difficulty: levels): NewStartingBoard => {
  const holes = difficultyLevels[difficulty] || difficultyLevels.medium;
  ...
  try {
    ...
    return newStartingBoard(difficulty);
  }
};
